...
const ViewBookings = () => {
  ...
  // Re-fetch booking data after successful reschedule
  useEffect(() => {
    if (isRescheduledConfirmed) {
      fetchBooking();
      setIsRescheduledConfirmed(false);
    }
  }, [isRescheduledConfirmed]);

  // Re-fetch booking data after successful cancellation
  useEffect(() => {
  ...
  const handleCancelBookingSuccess = (reason) => {
    ...
    setIsCancellationConfirmed(true); // Trigger re-fetch
    setShowCancelBookingModal(false); // Close the modal
  };
